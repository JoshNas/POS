...
class KitchenApplication(object):
    ...
    def delete(self, num):
        ...
        try:
            cur.execute("SELECT * FROM kitchen_orders WHERE number =?", (num,))
            row = cur.fetchone()
            ...
            cur.execute('DELETE FROM kitchen_orders WHERE number =?', (num,))
            conn.commit()
